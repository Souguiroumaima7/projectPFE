...
module.exports={
   ...
   req.body["image"] = req.file.filename
      ...
      const provider = new client_model(req.body)
      provider.save(req.body,(err,item)=>{
       if(err){
           res.status(406).json({message:"failed to save provider"+err})
       }else{
           res.status(201).json({message:" provider saved successuffly",data:item})
       }
      })
    }, 
       ...
       getall:(req,res)=>{
           provider_model.find({},(err,items)=>{
               if(err){
                   res.status(406).json({message:"failed to get all registred provider"})
               }else{
       ...
       getById:(req,res)=>{
           provider_model.findById(req.params.id,(err,item)=>{
               if(err){
                   res.status(406).json({message:"cannot get provider by this id"})
               }else{
       ...
       update:(req,res)=>{
           provider_model.findByIdAndUpdate(req.params.id,req.body,{new:true},(err,item)=>{
               if(err){
                   ...
                   res.status(200).json({message:"provider updated successuffly",data:item})
               }
           })
       },
       delete:(req,res)=>{
